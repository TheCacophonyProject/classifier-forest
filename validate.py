...
FEAT_LABELS = [
    'avg_sqrt_area', 'avg_elongation', 'avg_peak_snr', 'avg_mean_snr', 'avg_fill_factor', 'avg_move_1', 'avg_rel_move_1', 'avg_rel_x_move_1', 'avg_rel_y_move_1', 'avg_move_3', 'avg_rel_move_3', 'avg_rel_x_move_3', 'avg_rel_y_move_3', 'avg_move_5', 'avg_rel_move_5', 'avg_rel_x_move_5', 'avg_rel_y_move_5',
    'std_sqrt_area', 'std_elongation', 'std_peak_snr', 'std_mean_snr', 'std_fill_factor', 'std_move_1', 'std_rel_move_1', 'std_rel_x_move_1', 'std_rel_y_move_1', 'std_move_3', 'std_rel_move_3', 'std_rel_x_move_3', 'std_rel_y_move_3', 'std_move_5', 'std_rel_move_5', 'std_rel_x_move_5', 'std_rel_y_move_5',
    'max_sqrt_area', 'max_elongation', 'max_peak_snr', 'max_mean_snr', 'max_fill_factor', 'max_move_1', 'max_rel_move_1', 'max_rel_x_move_1', 'max_rel_y_move_1', 'max_move_3', 'max_rel_move_3', 'max_rel_x_move_3', 'max_rel_y_move_3', 'max_move_5', 'max_rel_move_5', 'max_rel_x_move_5', 'max_rel_y_move_5',
    'track_length'
]
...
y = num_classes * np.ones(num_samples)  # Anything that doesn't match one of the groups defined above will be given a high label, and we'll decide below whether to keep them or not
...
for train_index, test_index in kfold.split(X, y, I):
    ...
    p_pred = model.predict_proba(X_test)    # Probabilities are useful for filtering and generating ROC curves
...
for i in range(num_classes):
    for j in range(num_classes):
        s = np.sum(np.logical_and(actual_classes_masked==i, predicted_classes_masked==j))
        print(f'{s:9}', end='')
...
if num_classes == 2:
    ...
    # All predictions
    for i in range(2):
        RocCurveDisplay.from_predictions(actual_classes, predicted_prob[:,i], pos_label=i, name='ROC curve')
        plt.grid()
        plt.show()

    # Ignoring low probabilities
    for i in range(2):
        RocCurveDisplay.from_predictions(actual_classes_masked, predicted_prob_masked[:,i], pos_label=i, name='ROC curve (masked)')
        plt.grid()
        plt.show()
